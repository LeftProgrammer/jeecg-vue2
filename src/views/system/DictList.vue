<template>
	<a-card :bordered="false">
		...
		<div class="table-page-search-wrapper ">
			<a-form class='flex-between' layout="inline" @keyup.enter.native="searchQuery">
				...
				<div class="btn-left" style="border-top: 5px">
					<a-button style="background-color: #2c66f4" @click="handleAdd" type="primary" icon="plus">添加
					</a-button>
					<a-button style="background-color: #2c66f4" type="primary" icon="download"
						@click="handleExportXls('字典信息')">导出</a-button>
					<a-upload name="file" :showUploadList="false" :multiple="false" :headers="tokenHeader"
						:action="importExcelUrl" @change="handleImportExcel">
						<a-button style="background-color: #2c66f4" type="primary" icon="import">导入</a-button>
					</a-upload>
					<a-button style="background-color: #2c66f4" type="primary" icon="sync" @click="refleshCache()">刷新缓存
					</a-button>

					<a-button style="background-color: #2c66f4" type="primary" icon="hdd" @click="openDeleteList">回收站
					</a-button>
				</div>
			...
			<a-table :scroll='{y:574}' bordered ref="table" rowKey="id" size="middle" :columns="columns"
				:dataSource="dataSource" :pagination="ipagination" :loading="loading" @change="handleTableChange">
				<span slot="action" slot-scope="text, record">
					<a @click="handleEdit(record),modalForm=true">
						<a-icon type="edit" />
						编辑
					</a>
		...
		<!-- 字典类型 -->
		<dict-item-list v-show="showDictItemList" ref="dictItemList"></dict-item-list>
