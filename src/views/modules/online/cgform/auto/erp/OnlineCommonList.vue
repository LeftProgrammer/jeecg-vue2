  ...
  export default {
    ...
    methods:{
      initListByModel(hrefparam){
        ...
        this.table.relationType = this.model.relationType
        ...
        this.defColumns = currColumns;
        this.settingColumnsHandler(currColumns);
      ...
      //处理接收href参数
      handleAcceptHrefParams(hrefparam){
      ...
      initQueryInfo(){
        ...
        getAction(`${this.url.getQueryInfo}${this.code}`,param).then((res)=>{
      ...
      handleAdd(){
        if(this.table.relationType==1){
          if(this.table.dataSource && this.table.dataSource.length>0){
            this.$message.warning('一对一的表只能新增一条数据')
            return;
          }
        }
      ...
      handleDelete(id){
        let url = this.url.optPre+this.code+"/"+id+'?tabletype='+this.table.type
      ...
      handleExportXls(){
        ...
        let url = this.url.exportXls+this.code+'?tabletype='+this.table.type
        downFile(url,{paramsStr:paramsStr}).then((data)=>{
          if (!data || data.size===0) {
      ...
      initDependQueryParam(){
        if(this.table.type == 3){
          ...
          for(let item of this.foreignKeys){
            ...
            metaQueryParam['tabletype'] = this.table.type
      ...
      getImportUrl(){
        let url = '/online/cgform/api/importXls/'+this.code+'?tabletype='+this.table.type
        return url;
      },
      importOk(){
        this.loadData(1)
