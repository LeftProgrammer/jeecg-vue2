<template>
	<a-card :bordered="false">
				...
				<a-row :gutter="24">
					<a-col >
						...
						<a-form-item label="流程发起人">
							<a-input-search placeholder="请选择任务发起人" readOnly @search="handleSelect"
		...
		<!-- table区域-begin -->
		<a-table ref="table" size="middle" bordered rowKey="processInstanceId" :columns="columns"
			:dataSource="dataSource" :pagination="ipagination" :loading="loading" :scroll="{ y: 574}"
			:rowSelection="{selectedRowKeys: selectedRowKeys, onChange: onSelectChange}" @change="handleTableChange">
			<span slot="action" slot-scope="text, record">
				<a-popconfirm v-if="record.isSuspended!=''&& record.isSuspended==='true'" title="确定激活吗?"
					@confirm="() => handleRestart(record.id)"><a>启动</a></a-popconfirm>
				<a-popconfirm v-if="record.isSuspended!=''&& record.isSuspended==='false'" title="确定挂起吗?"
					@confirm="() => handleSuspend(record.id)"><a>挂起</a></a-popconfirm>
				...
				<a-dropdown>
					...
					<a-menu slot="overlay">
						<a-menu-item v-if="record.isSuspended!=''&& record.isSuspended!='finished'">
							<a-popconfirm title="确定关闭吗?" @confirm="() => handleClose(record.id)"><a>关闭</a>
							</a-popconfirm>
						</a-menu-item>
						<a-menu-item
							v-if="record.isSuspended!=''&& record.isSuspended==='false'&& record.isSuspended!='finished'">
							<a @click="selectEntruster(record)">委派</a></a-menu-item>
						<a-menu-item v-if="record.isSuspended!=''"><a @click="handleSkipNode(record.taskId)">跳转</a>
						</a-menu-item>
