  ...
  >
    <a-spin :spinning="confirmLoading">
      ...
      <map name="planetmap">
        <template>
          <area
            v-for="(item, key) in nodePositionInfo.positionList"
            :key="key"
            shape="rect"
            :coords="item.coords"
            title="Venus"
...
import qs from 'qs'
import NodeInfoModel from '@/views/modules/bpm/task/form/NodeInfoModel.vue'

export default {
  name: 'ExtActProcessInstPicModal',
  components: { NodeInfoModel },
  data() {
    return {
      title: '操作',
      visible: false,
  ...
  methods: {
    preview(flowCode, dataId) {
      ...
      getAction(this.url.getProcessInfo, params)
        .then((res) => {
          if (res.success) {
