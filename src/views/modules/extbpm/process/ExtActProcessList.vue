	...
	<a-card :bordered="false">
		...
		<div class="table-page-search-wrapper ">
			<a-form class='flex-between' layout="inline" @keyup.enter.native="searchQuery">
				...
				<!-- 操作按钮区域 -->
				<div class="">
					<a-button style="background-color: #2c66f4" @click="handleAdd" type="primary" icon="plus">新建流程
					</a-button>
		...
		<div class="mt-10">
			...
			<a-table ref="table" size="middle" bordered rowKey="id" :columns="columns" :dataSource="dataSource"
				...
				<span slot="action" slot-scope="text, record">
					<a-upload :showUploadList="false" :action="uploadAction(record.id)" :data="{'isup':1}"
						:headers="tokenHeader" @change="handleChange">
						<a>上传流程</a>
					</a-upload>
	...
	export default {
		...
		data() {
			return {
				...
				url: {
					list: "/act/process/extActProcess/list",
		...
		methods: {
			...
			handlePublish: function(id) {
				...
				this.$confirm({
					...
					content: "是否发布该流程?",
					onOk: function() {
						putAction(that.url.deployProcess, {
							id: id
						}).then((res) => {
							if (res.success) {
			...
			copyProcess: function(id) {
				...
				this.$confirm({
					...
					content: "是否复制该流程?",
					onOk: function() {
						getAction(that.url.copyProcess, {
							id: id
						}).then((res) => {
							if (res.success) {
								that.$message.success(res.message);
