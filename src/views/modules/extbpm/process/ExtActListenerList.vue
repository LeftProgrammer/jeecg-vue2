	...
	<a-card :bordered="false">
      ...
      <a-form layout="inline">
		...
		<div class="table-operator">
			<a-button style="background-color: #2c66f4" @click="handleAdd" type="primary" icon="plus">新增</a-button>
		...
		<div>
			...
			<a-table :scroll="{ y: 500 }" ref="table" size="middle" bordered rowKey="id" :columns="columns" :dataSource="dataSource"
				...
				<span slot="action" slot-scope="text, record">
					...
					<a-popconfirm title="确定启用吗?" v-if="record.listenerStatus==0"
						@confirm="() => handleStatus(record.id)">
						<a>启用</a>
					</a-popconfirm>
					<a-popconfirm title="确定禁用吗?" v-if="record.listenerStatus==1"
						@confirm="() => handleStatus(record.id)">
						<a>禁用</a>
					</a-popconfirm>
					<a-divider type="vertical" />
	...
	export default {
		...
		data() {
			return {
				...
				url: {
					...
					deleteBatch: "/act/process/extActListener/deleteBatch",
		...
		methods: {
			handleStatus: function(id) {
				var that = this;
				putAction(that.url.changeStatus, {
					id: id
				}).then((res) => {
					if (res.success) {
