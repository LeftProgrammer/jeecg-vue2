        ...
        <div style="overflow-y: auto; overflow-x: hidden; max-height: 300px" v-if="sub.relationType == 1">
          <j-form-container :disabled="online_form_disabled">
            <online-sub-form
              ...
              @executeFillRule="handleSubFormExecuteFillRule(sub.key)"
            />
          </j-form-container>
        </div>
        <j-vxe-table
...
import { getRefPromise, validateFormAndTables, VALIDATE_FAILED } from '@/components/jeecg/JVxeTable/utils/vxeUtils.js'
...
import { addOnlineBtAuth2Storage } from '@/utils/authFilter'
...
export default {
  ...
  methods: {
    createRootProperties() {
      ...
      Object.keys(properties).map((key) => {
        ...
        if (item.view == 'tab') {
          ...
          if (item.relationType == 1) {
            ...
            //update--begin--autor:lvdandan-----date:20201203------for：JT-55【online表单】JEditable 有单选组件时，样式问题
            //如果是一对多子表处理单选组件改为下拉框
            this.dealSubProerties(item)
            //update--end--autor:lvdandan-----date:20201203------for：JT-55【online表单】JEditable 有单选组件时，样式问题
      ...
      this.subDataSource = {}
      subDataSourceKeys
        .sort((one, next) => one.order - next.order)
        .forEach((item) => {
    ...
    add(param) {
      ...
      Object.keys(this.subDataSource).forEach((key) => {
        this.$set(this.subDataSource, key, [])
      })
      this.$nextTick(() => {
        if (param) {
    ...
    initFormData(record) {
      ...
      return new Promise((resolve) => {
        ...
        this.$nextTick(() => {
          ...
          Object.keys(this.subDataSource).forEach((key) => {
            this.$set(this.subDataSource, key, this.model[key])
          })
    ...
    //处理js增强 自定义 提交前事件
    customBeforeSubmit(formData) {
      if (this.EnhanceJS && this.EnhanceJS['beforeSubmit']) {
        return this.EnhanceJS['beforeSubmit'](this, formData)
      } else {
    ...
    doApplyRequest(formData) {
      ...
      let url = this.url + '?tabletype=' + this.tableType
      httpAction(url, formData, this.submitMethod).then((res) => {
        console.log(res)
        if (res.success) {
          //console.log('formData',formData)
          if (res.result) {
            formData['flow_submit_id'] = res.result
          }
          if (this.tree === true) {
    ...
    handleApplyRequest(formData) {
      this.customBeforeSubmit(formData)
        .then(() => {
          this.doApplyRequest(formData)
        })
        .catch((msg) => {
        ...
        .then((allValues) => {
          ...
          if (allValues.tablesValue) {
            ...
            for (let i = 0; i < keys.length; i++) {
              formData[keys[i]] = allValues.tablesValue[i].tableData
            }
          }
    ...
    //处理一对一子表的表单改变事件
    handleSubFormChange(valueObj, tableKey) {
      if (this.EnhanceJS && this.EnhanceJS[tableKey + '_onlChange']) {
        let tableChangeObj = this.EnhanceJS[tableKey + '_onlChange']()
        let columnKey = Object.keys(valueObj)[0]
        if (tableChangeObj[columnKey]) {
          let subRef = this.$refs[tableKey]
    ...
    addSubRows(subKey, rows) {
      ...
      if (typeof rows == 'object') {
        //lvdandan --begin-- date:20201117 for:LOWCOD-987 【online】新版行编辑js增强的问题
        this.$refs[subKey][0].addRows(rows, true)
        //lvdandan --end-- date:20201117 for:LOWCOD-987 【online】新版行编辑js增强的问题
      } else {
        this.$message.error('你脑壳瓦钓了,传的什么参数!')
      }
    },
    clearSubRows(subKey) {
      //update-begin-author:lvdandan date:20201105 for:LOWCOD-987 【online】js增强的问题--删除数据时获取所有数据
      let rows = [...this.$refs[subKey][0].getNewDataWithId(), ...this.subDataSource[subKey]]
      //update-end-author:lvdandan date:20201105 for:LOWCOD-987 【online】js增强的问题--删除数据时获取所有数据
      ...
      this.$refs[subKey][0].removeRowsById(ids)
    },
    ...
    checkOnlyFieldValue(rule, value, callback) {
      ...
      //对于视图 需要将表名后的$+数字 取掉
      let realTableName = this.tablename.replace(/\$\d+/, '')
      let param = {
      ...
      loadFieldDefValForSubTable({
        ...
        getFormData: () => {
          ...
          if (subForms.form) {
            ...
            sub = subForms.jvt.getTableData({ rowIds: [row.id] })[0]
          }
      ...
      this.$nextTick(() => {
        this.subTabInfo.forEach((subTable) => {
          if (subTable.key === subKey) {
            ...
            this.loadSubFieldDefVal({ jvt: target, form }, subTable, row, ACTION_TYPES.RELOAD)
          }
        })
      })
    },
    ...
    handleExecuteFillRuleSub(subTable, event) {
      let { row, form, target } = event
      this.loadSubFieldDefVal({ jvt: target, form }, subTable, row, ACTION_TYPES.ADD)
    },
    //子表表单设置默认值
    handleSubFormExecuteFillRule(key) {
      let form = this.$refs[key][0].form
      loadFieldDefVal({
        form: form,
        properties: this.subProperties[key],
      ...
      this.$nextTick(() => {
        ...
        setTimeout(() => {
          ...
          if (obj) {
            ...
            let num =
              Number(getOffsetTopByBody(obj)) - Number(getOffsetTopByBody(modal)) + 480 - Number(modal.offsetWidth)
            if (num > 0) {
    ...
    getSubTableAuthPre(table) {
      return 'online_' + table + ':'
    },
    handleSubTableButtonAuth(item) {
      let pre = 'online_' + item.key + ':'
      addOnlineBtAuth2Storage(pre, item.hideButtons)
    },
  },
}
</script>
