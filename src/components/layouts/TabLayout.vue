...
import Vue from 'vue'
import { CACHE_INCLUDED_ROUTES, INDEX_MAIN_PAGE_PATH } from '@/store/mutation-types'
...
export default {
  ...
  created() {
    if (this.$route.path !== INDEX_MAIN_PAGE_PATH) {
  ...
  watch: {
    $route: function (newRoute) {
      ...
      } else if (INDEX_MAIN_PAGE_PATH == newRoute.fullPath) {
        //首页时 判断是否缓存 没有缓存 刷新之
    ...
    device() {
      if (this.multipage && this.linkList.indexOf(INDEX_MAIN_PAGE_PATH) === -1) {
        this.addIndexToFirst()
      }
    },
  ...
  methods: {
    ...
    addIndexToFirst() {
      ...
      this.linkList.splice(0, 0, INDEX_MAIN_PAGE_PATH)
    },
    // update-end-author:sunjianlei date:20191223 for: 修复从单页模式切换回多页模式后首页不居第一位的 BUG

    // update-begin-author:sunjianlei date:20200120 for: 动态更改页面标题
    changeTitle(title) {
      let projectTitle = '鲸禾JMC后台管理系统'
      // 首页特殊处理
      if (this.$route.path === INDEX_MAIN_PAGE_PATH) {
    ...
    closeOthers(pageKey) {
      ...
      if (pageKey == INDEX_MAIN_PAGE_PATH || pageKey.indexOf('?ticke=') >= 0) {
    ...
    closeLeft(pageKey) {
      if (pageKey == INDEX_MAIN_PAGE_PATH) {
        return
      }
