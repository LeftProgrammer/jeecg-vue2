...
<template>
  <a-spin :spinning="loading">
    <a-row type="flex">
      ...
      <a-col :span="12">
        <!-- 操作按钮 -->
        <div v-if="actionButton" class="action-button" :class="disabled ? 'action-disabled' : ''">
          <a-button v-if="buttonPermission('add')" type="" icon="" @click="handleClickAdd" :disabled="disabled"
            >添加
          </a-button>
          <span class="gap"></span>
          <template v-if="!deleteInvisible && selectedRowIds.length > 0">
            <a-popconfirm :title="`确定要删除这 ${selectedRowIds.length} 项吗?`" @confirm="handleConfirmDelete">
              <a-button v-if="buttonPermission('batch_delete')" type="" icon="" :disabled="disabled">删除</a-button>
              <span class="gap"></span>
            </a-popconfirm>
              ...
              >
                ...
                >
                  ...
                  <template v-for="(id, i) in [`${col.key}${row.id}`]">
                    ...
                    <template v-else-if="col.type === formTypes.date || col.type === formTypes.datetime">
                      <a-tooltip v-bind="buildTooltipProps(row, col, id)">
                        <j-date
                          ...
                          :dateFormat="col.type === formTypes.date ? 'YYYY-MM' : 'YYYY-MM-DD HH:mm:ss'"
                          :disabledDate="
                            col.disabledDate
                              ? (v) => {
                                  return handleDisabledDate(v, row, col)
                                }
                              : null
                          "
                          allowClear
                    ...
                    <!-- month -->
                    <template v-else-if="col.type === formTypes.month">
                      <a-tooltip v-bind="buildTooltipProps(row, col, id)">
                        <a-month-picker
                          v-if="isEditRow(row, col)"
                          :id="id"
                          ...
                          "
                          allowClear
                          @change="(v) => handleChangeAMonthChange(v, id, row, col)"
                        />
                        <span
                          v-else
                          class="j-td-span no-edit"
                          :class="{ disabled: buildProps(row, col).disabled }"
                          @click="handleEditRow(row, col)"
                          >{{ jmonthValues[id] }}</span
                        >
                      </a-tooltip>
                    </template>
                    <!-- tree -->
                    <template v-else-if="col.type === formTypes.sel_tree">
                      <a-tooltip v-bind="buildTooltipProps(row, col, id)">
                        <risklde-ass-sel-tree
                          v-if="isEditRow(row, col)"
                          :id="id"
                          :key="i"
                          v-bind="buildProps(row, col)"
                          :value.sync="aTreeValue[id]"
                          style="width: 100%"
                          :getCalendarContainer="getParentContainer"
                          :placeholder="replaceProps(col, col.placeholder)"
                          :disabled="disabled"
                          allowClear
                          @change="(v) => riskldeChange(v, id, row, col)"
                        ></risklde-ass-sel-tree>
                        <span
                          v-else
                          class="j-td-span no-edit"
                    ...
                    <div v-else-if="col.type === formTypes.file" :key="i">
                      <template v-if="uploadValues[id] != null">
                        <div v-for="(file, fileKey) of [uploadValues[id] || {}]">
                          <div :key="fileKey" style="position: relative">
                            ...
                            <a-tooltip v-else-if="file.status === 'done'" :title="getFilename(file.name)">
                              <a-icon type="paper-clip" />
                              <span style="margin-left: 5px">{{ getEllipsisWord(file.name, 5) }}</span>
                            </a-tooltip>

                            <a-tooltip v-else :title="getFilename(file.name)">
                            ...
                            <template style="width: 30px">
                              ...
                              >
                                ...
                                <a-menu slot="overlay">
                                  <a-menu-item
                                    v-if="col.allowPreview !== false"
                                    @click="handleClickPreviewFileByUrl(id)"
                                  >
                                  ...
                                  <a-menu-item v-if="!disabled" @click="handleClickDelFile(id)">
                                    <span><a-icon type="delete" />&nbsp;删除</span>
                                  </a-menu-item>
                                  <a-menu-item v-if="!disabled" @click="handleMoreOperation(id, col, col)">
                      ...
                      <div :hidden="uploadValues[id] != null">
                        <a-tooltip v-if="!disabled" v-bind="buildTooltipProps(row, col, id)">
                          <a-upload
                            name="file"
                            :data="{ isup: 1 }"
                            :multiple="false"
                      ...
                      <div v-if="disabled && uploadValues[id] == null">无</div>
                    </div>
                    ...
                    <!-- else (normal) -->
                    <span v-else :key="i" v-bind="buildProps(row, col)" :title="inputValues[rowIndex][col.key]">{{
                      inputValues[rowIndex][col.key]
                    }}</span>
                  </template>
                </div>
...
export default {
  name: 'JEditableTable',
  components: { JDate, Draggable, JInputPop},
  provide() {
    return {
  ...
  methods: {
    ...
    getFilename(url) {
      if (url) {
        ...
        if (lineIndex > 0 && pointIndex > lineIndex) {
          name = name.substr(0, lineIndex) + name.substr(pointIndex, len - pointIndex)
        }
        return name
      } else {
        return ''
    ...
    _pushByDataSource(dataSource, insertIndexes = null, update = true, rows = null, setDefaultValue = false) {
      ...
      let jmonthValues = { ...this.jmonthValues }
      let aTreeValue = { ...this.aTreeValue }
        ...
        this.columns.forEach((column) => {
          ...
          if (setDefaultValue) {
            ...
            if (defaultValue instanceof Function) {
              defaultValue = defaultValue(newValueIndex)
            }
          ...
          } else if (column.type === FormTypes.month) {
            jmonthValues[inputId] = sourceValue
          } else if (column.type === FormTypes.sel_tree) {
            aTreeValue[inputId] = sourceValue
      ...
      this.jmonthValues = jmonthValues
      this.aTreeValue = aTreeValue
      ...
      for (let value of inputValues) {
        ...
        this.columns.forEach((column) => {
          ...
          if (column.type === FormTypes.checkbox) {
            ...
            value[column.key] = this.jmonthValues[inputId]
          } else if (column.type === FormTypes.sel_tree) {
            value[column.key] = this.aTreeValue[inputId]
          } else if (column.type === FormTypes.sel_depart) {
    ...
    _getAllValuesForSlot() {
      return cloneObject({
        ...
        jmonthValues: this.jmonthValues,
        aTreeValue: this.aTreeValue,
              ...
              if (column.key === newValueKey) {
                ...
                if (!edited) {
                  ...
                  if (column.type === FormTypes.select) {
                    ...
                    edited = this.setOneValue(this.jmonthValues, modelKey, newValue)
                  } else if (column.type === FormTypes.sel_tree) {
                    edited = this.setOneValue(this.aTreeValue, modelKey, newValue)
                  } else if (column.type === FormTypes.sel_depart) {
    ...
    validateOneInputAsync(value, row, column, notPassedIds, update = false, validType = 'input', callback) {
      ...
      let [passed, message] = this.validateValue(column, value, row)
    ...
    /** 通过规则验证值是否正确 */
    validateValue(column, value, row) {
    ...
    recalcOneStatisticsColumn(key) {
      if (this.hasStatisticsColumn) {
        if (this.statisticsColumns.hasOwnProperty(key)) {
          ...
          this.statisticsColumns[key] = Number.parseFloat(Number.parseFloat(count).toFixed(2))
        }
      }
    },
    ...
    riskldeChange(value, id, row, column) {
      this.aTreeValue = this.bindValuesChange(value, id, 'aTreeValue')
      this.validateOneInput(value, row, column, this.notPassedIds, true, 'change')
      // 触发valueChange 事件
      this.elemValueChange(FormTypes.sel_tree, row, column, value)
    },
    handleConfirmDelete() {
    ...
    //列表中选择时间控件，不可选择属性的控制
    handleDisabledDate(value, row, col) {
      if (col.disabledDate instanceof Function) {
        return col.disabledDate(value, row, col)
      }
    ...
    handleChangeAMonthChange(value, id, row, column) {
      ...
      this.jmonthValues = this.bindValuesChange(value, id, 'jmonthValues')
      this.validateOneInput(value, row, column, this.notPassedIds, true, 'change')
      // 触发valueChange 事件
      this.elemValueChange(FormTypes.month, row, column, value)
    },
    //部门组件值改变
    ...
    async handleClickPreviewFileByUrl(id) {
      let { url, path } = this.uploadValues[id] || {}
      let prevurl = ''
      if (!url || url.length === 0) {
        if (path && path.length > 0) {
          prevurl = await getFilePreviewUrl(path.split(',')[0])
        }
      }
      if (
        prevurl &&
        typeof prevurl === 'string' &&
        (prevurl.toLowerCase().startsWith('http') || prevurl.toLowerCase().startsWith('https'))
      ) {
        window.open(prevurl)
      }
    },
    ...
    buildTdStyle(col) {
      ...
      if (col.width) {
        ...
        var iwidth = 0
        var pwidth = 0
        if (this.rowNumber) iwidth += 45
        if (this.rowSelection) iwidth += 45
        if (this.actionButton) iwidth += 45
        for (let i = 0; i < this.columns.length; i++) {
          const column = this.columns[i]
          if (column.width) {
            try {
              if (column.width.endsWith('%')) {
                pwidth += parseFloat(column.width.substr(0, column.width.length - 1))
              }
              if (column.width.endsWith('px')) {
                iwidth += parseFloat(column.width.substr(0, column.width.length - 2))
              }
            } catch (er) {
              console.warn('列宽计算错误：' + column.width)
            }
          }
        }
        var iEmptyColumnCount = this.columns.filter((column) => !column.width).length
        style['width'] = `calc(${(100 - pwidth) / iEmptyColumnCount}% - ${iwidth / iEmptyColumnCount}px)`
      } else {
        style['width'] = '120px'
      }
    ...
    getEllipsisWord(content, len) {
      content = this.getFilename(content)
