<template>
	<a-spin tip="Loading..." :spinning='spinning'>
		<div :id="containerId" class="jupload-container" :class="disabled ? 'disabled' : ''">
			...
			<a-upload name="file" :multiple="true" :action="uploadAction" :headers="headers" :data="{ biz: bizPath }"
				:fileList="fileList" :beforeUpload="beforeUpload" @change="handleChange" :disabled="disabled"
				:returnUrl="returnUrl" :listType="complistType" @preview="handlePreview"
				:class="{ 'uploadty-disabled': disabled }" class="uploadty_box">
				<template>
			...
			<div v-if="fileList && fileList.length > 0"
				class="ant-upload-list ant-upload-list-text jinghe-ant-upload-list">
				<div class="ant-upload-list-item ant-upload-list-item-done ant-upload-list-item-list-type-text"
					v-for="(file, i) in fileList" :key="file.uid">
					<div class="ant-upload-list-item-info">
						<a-icon type="paper-clip" />
						<a target="_blank" rel="noopener noreferrer" :title="'下载： ' + file.name" :href="file.url"
							class="ant-upload-list-item-name ant-upload-list-item-name-icon-count-1">{{ getFilename(file.name) }}</a>
						...
						<a target="_blank" rel="noopener noreferrer" :title="'预览： ' + file.name" :href="file.prevurl"
							class="previewbtn">预览</a>
...
<script>
	...
	import SplitPanelVue from '../../views/jeecg/SplitPanel.vue'
