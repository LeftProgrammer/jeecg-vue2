<template>
	<a-tree-select allowClear labelInValue :getPopupContainer="(node) => node.parentNode" style="width: 100%"
		:disabled="disabled" :dropdownStyle="{ maxHeight: '400px', overflow: 'auto' }" :placeholder="placeholder"
		:loadData="asyncLoadTreeData" :value="treeValue" :treeData="treeData" :multiple="multiple" @change="onChange"
		@search="onSearch">
	</a-tree-select>
</template>
<script>
			...
			asyncLoadTreeData(treeNode) {
				return new Promise((resolve) => {
					...
					getAction(this.url, param).then((res) => {
						// 这里写了一个  disabled 通过父子传值控制 父节点是否能被选中
						if (res.success) {
