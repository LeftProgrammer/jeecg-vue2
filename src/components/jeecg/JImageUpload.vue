  ...
  <div class="img">
    ...
    >
      <div :style="{ width: !isMultiple ? '104px' : 'auto', height: !isMultiple ? '104px' : 'auto' }">
        <img v-if="!isMultiple && picUrl" :src="getImgThumbnailUrl()" style="width: 100%; height: 100%" />
    ...
    <!-- <viewer :images="fileList.map((x) => x.url)" class="image_view ant-upload-list">
      <div class="ant-upload-list-picture-card-container">
        <img v-for="src in fileList.map((x) => x.url)" :src="src" :key="src" />
      ...
      handler(val, oldValue) {
        if (val instanceof Array && val.length > 0) {
          if (this.fileList.length > 0) {
            var listdom = document.querySelector('.img .ant-upload-list')
            if (listdom) {
  ...
  methods: {
    ...
    initFileList(paths) {
      ...
      for (var a = 0; a < arr.length; a++) {
        ...
        let thumbUrl = getImageThumbnailUrl(url)
        fileList.push({
          uid: uidGenerator(),
    ...
    getImgThumbnailUrl(text) {
      text = getImageThumbnailUrl(text)
