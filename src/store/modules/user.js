      ...
      return new Promise((resolve, reject) => {
        getAction("/sys/cas/client/validateLogin", userInfo).then(response => {
          ...
          if (response.success) {
            ...
            userInfo.isAdmin = userInfo.username == 'admin'
      ...
      return new Promise((resolve, reject) => {
        login(userInfo).then(response => {
          if (response.code == '200') {
            const result = response.result
            const userInfo = result.userInfo
            if (!userInfo.orgCodeTxt && result.departs && result.departs.length > 0 && userInfo.orgCode) {
              var depart = result.departs.find(x => x.orgCode == userInfo.orgCode);
              if (depart) {
                userInfo.orgCodeTxt = depart.departName;
              }
            }
            userInfo.isAdmin = userInfo.username == 'admin'
            Vue.ls.set(ACCESS_TOKEN, result.token, 7 * 24 * 60 * 60 * 1000)
